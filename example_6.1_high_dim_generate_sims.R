...
run_sims <- function( num_sims ) {
  ...
  rejections_fd_pv_proj <- matrix(0, ncol = num_sims * length(ns), nrow = 6)
  rejections_fd_pv_no_proj <- matrix(0, ncol = num_sims * length(ns), nrow = 6)
  rejections_fd_pv_no_proj_lambda <- matrix(0, ncol = num_sims * length(ns), nrow = 6)
  ...
  for (k in 1:length(ns)) {
    ...
    for (i in 1:num_sims) {
      ...
      rejections_fd_pv_no_proj[1, (k-1)*num_sims + i] <- PermVarTest(X, Y, sample_Rh, a_B, g_dot, plot_flag = FALSE, m_lambda = a_lambda_no_proj)
      rejections_fd_pv_no_proj[2, (k-1)*num_sims + i] <- PermVarTest(X, Y, sample_Rv, a_B, g_dot, m_lambda = a_lambda_no_proj)
      rejections_fd_pv_no_proj[3, (k-1)*num_sims + i] <- PermVarTest(X, Y, sample_R180, a_B, g_dot, m_lambda = a_lambda_no_proj)
      rejections_fd_pv_no_proj[4, (k-1)*num_sims + i] <- PermVarTest(X, Y, sample_Rforwardslash, a_B, g_dot, m_lambda = a_lambda_no_proj)
      rejections_fd_pv_no_proj[5, (k-1)*num_sims + i] <- PermVarTest(X, Y, sample_Rbackslash, a_B, g_dot, m_lambda = a_lambda_no_proj)
      rejections_fd_pv_no_proj[6, (k-1)*num_sims + i] <- PermVarTest(X, Y, sample_R90, a_B, g_dot, m_lambda = a_lambda_no_proj)
      
      rejections_fd_pv_no_proj_lambda[1, (k-1)*num_sims + i] <- PermVarTest(X, Y, sample_Rh, a_B, g_dot, plot_flag = FALSE, m_lambda = a_lambda_proj)
      rejections_fd_pv_no_proj_lambda[2, (k-1)*num_sims + i] <- PermVarTest(X, Y, sample_Rv, a_B, g_dot, m_lambda = a_lambda_proj)
      rejections_fd_pv_no_proj_lambda[3, (k-1)*num_sims + i] <- PermVarTest(X, Y, sample_R180, a_B, g_dot, m_lambda = a_lambda_proj)
      rejections_fd_pv_no_proj_lambda[4, (k-1)*num_sims + i] <- PermVarTest(X, Y, sample_Rforwardslash, a_B, g_dot, m_lambda = a_lambda_proj)
      rejections_fd_pv_no_proj_lambda[5, (k-1)*num_sims + i] <- PermVarTest(X, Y, sample_Rbackslash, a_B, g_dot, m_lambda = a_lambda_proj)
      rejections_fd_pv_no_proj_lambda[6, (k-1)*num_sims + i] <- PermVarTest(X, Y, sample_R90, a_B, g_dot, m_lambda = a_lambda_proj)
            
      X_proj <- cbind( X[,1:2], matrix(0, nrow = n, ncol = dimension - 2) )
      
      rejections_fd_pv_proj[1, (k-1)*num_sims + i] <- PermVarTest(X_proj, Y, sample_Rh, a_B, g_dot, m_lambda = a_lambda_proj)
      rejections_fd_pv_proj[2, (k-1)*num_sims + i] <- PermVarTest(X_proj, Y, sample_Rv, a_B, g_dot, m_lambda = a_lambda_proj)
      rejections_fd_pv_proj[3, (k-1)*num_sims + i] <- PermVarTest(X_proj, Y, sample_R180, a_B, g_dot, m_lambda = a_lambda_proj)
      rejections_fd_pv_proj[4, (k-1)*num_sims + i] <- PermVarTest(X_proj, Y, sample_Rforwardslash, a_B, g_dot, m_lambda = a_lambda_proj)
      rejections_fd_pv_proj[5, (k-1)*num_sims + i] <- PermVarTest(X_proj, Y, sample_Rbackslash, a_B, g_dot, m_lambda = a_lambda_proj)
      rejections_fd_pv_proj[6, (k-1)*num_sims + i] <- PermVarTest(X_proj, Y, sample_R90, a_B, g_dot, m_lambda = a_lambda_proj)
    ...
    print( rowMeans( rejections_fd_pv_no_proj[,((k-1)*num_sims + 1):(k*num_sims)] > alpha ) )
    print("Permutation Variant with Nonzero Lambda")
    print( rowMeans( rejections_fd_pv_no_proj_lambda[,((k-1)*num_sims + 1):(k*num_sims)] > alpha ) )
    print("Projected Data Permutation Variant")
    print( rowMeans( rejections_fd_pv_proj[,((k-1)*num_sims + 1):(k*num_sims)] > alpha ) )
    print( " " )
  }
  write.csv( rejections_fd_avt, file = avt.file.name)
  write.csv( rejections_fd_pv_no_proj, file = pvt_no_proj.file.name )
  write.csv( rejections_fd_pv_no_proj_lambda, file = pvt_no_proj_lambda.file.name)
  write.csv( rejections_fd_pv_proj, file = pv_with_proj.file.name )
